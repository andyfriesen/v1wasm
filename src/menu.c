...
drawloop:
  ...
  gotoxy(42,126);  printstring("- Equipment -");
  gotoxy(27,136);  printstring(items[pstats[c].inv[0]].name);
  gotoxy(27,146);  printstring(items[pstats[c].inv[1]].name);
  gotoxy(27,156);  printstring(items[pstats[c].inv[2]].name);
  gotoxy(27,166);  printstring(items[pstats[c].inv[3]].name);
  gotoxy(27,176);  printstring(items[pstats[c].inv[4]].name);
  gotoxy(27,186);  printstring(items[pstats[c].inv[5]].name);
...
drawloop:
  ...
  if (b1)
  {
    if (!p) { playeffect(4); gp+=items[pstats[l].inv[ptr]].price/2;
...
//  PutEquipBox(partyidx[c]-1);
  ...
  if (b1)
  {
    if (!p) { playeffect(4); gp+=magic[pstats[l].maginv[ptr]].cost/2;
              RemoveMagic(l,ptr);}
    first=2;
  }
...
drawloop:
 ...
 if (up)     { if (my) my--;
  ...
  if (b1)    {
               if (storeinv[ptr])
               {
                   if (gp<magic[storeinv[ptr]].cost) playeffect(3);
                   else BuyMagic(ptr);
               }
               first=1;
             }
