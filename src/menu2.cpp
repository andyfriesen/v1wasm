...
{ unsigned char l,i,a,*img,z;
  ...
  a=pstats[l].inv[ptr];
  if (!items[a].useflag || items[a].useflag>=5) fontcolor(17);
...
{ unsigned char l,i,a,*img,z;
  ...
  if (!magic[a].useflag || magic[a].useflag>=5) fontcolor(17);
...
{ int first=1, ptr=0,i,j;
  ...
  if (!pstats[a].curhp) {playeffect(3); return;}
  if (numchars==1)
     { l=a; goto usesec; }
