...
drawloop:
    ...
    if (first==2) if (b1 || b2 || b4) goto drawloop;
            ...
            i=0;
            while (pstats[partyidx[i]-1].invcnt==24) i++;
...
EarthQuake() {
    ...
    while (timer_count<=i) {
        ...
        if (!switchflag) {
            if (k>xwin && k) nxw=0;
            else if (k) nxw=xwin-(rand() % k);
            if (j>ywin && j) nyw=0;
            else if (j) nyw=ywin-(rand() % j);
        } else {
...
GetNextMove() {
    ...
    case 10:
        ...
        if (party[0].cx==party[0].cmdarg) party[0].cmdarg=0;
        break;
        break;
    case 11:
        ...
        if (party[0].cy==party[0].cmdarg) party[0].cmdarg=0;
        break;
        break;
    case 12:
...
int ReadVar1(int var, int arg1) {
    ...
    case 0:
        return flags[arg1];
    case 1:
        if (party[0].facing==arg1) return 1;
        else return 0;
    case 2:
        for (i=0; i<numchars; i++)
